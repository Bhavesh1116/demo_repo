...
// Safely initialize. If key is missing/empty, ai will be null, preventing crash on load.
...
export const askRestaurantAgent = async (question: string) => {
  if (!ai) return "I can't connect to live maps right now (API Key missing), but we are located at " + RESTAURANT_INFO.address;
...
export const getLiveStatus = async (): Promise<{text: string, source: string | null}> => {
   if (!ai) return { text: "Live satellite feed unavailable. Expect the usual line!", source: null };
   ...
   try {
     ...
     if (chunks) {
        ...
        // Use optional chaining to safely access 'maps' property
